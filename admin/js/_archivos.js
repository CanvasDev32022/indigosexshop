...
const cargar_archivos = (item, tamanio="imagena") => {
			...
			<div class="modal-footer">
				<div class="row m-0">
					<div class="col s12 m4 offset-m8 nput-field">
						...
						<button type="submit" id="action_${seccion_singular}" class="btn waves-effect waves-light azulclaro">SUBIR</button>
					</div>
				</div>
			</div>
		</form>`;
	...
	M.updateTextFields();
	validacion_documento(item, tamanio);
...
const cargar_archivo_thumb = (archivo, titulo, ruta="", item) => {

	const extension = archivo.split(".")[1];
	let contenido = "";
	contenido = document.getElementById(`archivosC-${item}`).innerHTML;

	contenido = `
		<a class="archivo" id="${archivo.split(".")[0]}">
			<span class="borrable" onclick="eliminar_archivo(this, ${item})" idC="${archivo.split(".")[0]}" archivo="${ruta}${archivo}"><i class="material-icons">close</i></span>
	...
	document.getElementById(`archivosC-${item}`).innerHTML = contenido;
}

// TODO: FUNCION PARA ELIMINAR ARCHIVO
const eliminar_archivo = (cmp, item) => {
