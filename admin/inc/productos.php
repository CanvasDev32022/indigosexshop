<?php 
	...
	elseif($_POST['action'] == "crear")
	{
		...
		if($pctS < 0) {
			...
			$prepare = "
				INSERT INTO productos (prd_imagen, prd_nombre, prd_descripcion_corta, prd_descripcion_larga, prt_id, prd_visible, pct_ids, prd_precio, prd_promocion, prd_porcentajepromocion, prd_preciopromocion, prs_id, prd_destacado, prd_nuevo, prd_referencia, prd_padre, prd_relacionado, prd_metatitulo, prd_metadescripcion, prd_metakeywords, prd_metaimagenc, prd_metaimagend, prd_creado)
				VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$params = [
			...
			$types = ['s','s','s','s','i','i','s','d','i','d','d','i','i','i','s','i','s','s','s','s','s','s','s'];
	...
	{
		$prepare = "SELECT prd_id, prd_imagen, prd_nombre, prd_descripcion_corta, prd_descripcion_larga, prt_id, prd_visible, pct_ids, prd_precio, prd_promocion, prd_porcentajepromocion, prd_preciopromocion, prs_id, prd_destacado, prd_nuevo, prd_referencia, prd_padre, prd_relacionado, prd_metatitulo, prd_metadescripcion, prd_metakeywords, prd_metaimagenc, prd_metaimagend FROM productos WHERE prd_id = ?";
