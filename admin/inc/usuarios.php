<?php 
	...
	if($_POST['action'] == "lista") 
	{
		...
		else
		{
			...
			if($registros == 0) {
				...
				$prepare 	= "SELECT usr_id, usr_nombres, usr_apellidos, rol.rol_nombre, usr.est_id FROM usuarios usr INNER JOIN roles rol ON rol.rol_id = usr.rol_id INNER JOIN _estados est ON est.est_id = usr.est_id WHERE usr_borrado = 0 AND (usr_nombres LIKE ? OR usr_apellidos LIKE ? OR rol.rol_nombre LIKE ? OR est.est_nombre_$idioma LIKE ?) ORDER BY usr_nombres ASC LIMIT $inferior, $max ";
				$params 	= [$busqueda, $busqueda, $busqueda, $busqueda];
				$types		= ['s', 's', 's','s'];
				$usrS		= $toolSQL->selectSQL($prepare, $types, $params);
				if($usrS < 0)
	...
	{
		$idioma = $_POST['idioma'];
		$prepare 	= "SELECT rol_id, rol_nombre FROM roles WHERE ?";
		$params 	= [1];
		$types		= ['i'];
	...
	{
		$prepare = "SELECT COUNT(1) AS conteo FROM usuarios WHERE usr_email = ? AND usr_borrado = 0";
	...
	{
		$prepare = "UPDATE usuarios SET usr_borrado = 1 WHERE usr_id = ?";
		$params = [intval($_POST['id'])];
		$types = ['i'];
